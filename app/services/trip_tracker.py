...
async def save_trip(user_email: str, trip_data: dict):
    """
    Save a completed shopping trip and update all intelligence collections.
    This is the core data loop - every trip makes CartWise smarter.
    """
    db = get_db()
...
async def update_item_pattern(db, user_id: str, item: dict):
    """
    Update buying pattern for this item. Tracks frequency, preferred brand/store, avg price.
    """
...
async def update_price_history(db, item: dict):
    """
    Track price over time for each product at each store.
    Used for price alerts and trend detection.
    """
...
async def update_user_regulars(db, email: str, items: list):
    """
    Auto-learn regular items from shopping history.
    If user buys something 3+ times, add it to regulars.
    """
...
async def get_smart_recommendations(user_email: str):
    """
    Generate smart recommendations based on purchase patterns.
    - Items due for restock
    - Price drops on items they buy
    - Suggested items based on frequency
    """
    db = get_db()
    user = await db.users.find_one({"email": user_email})
    if not user:
    ...
    for pattern in patterns:
        ...
        if next_expected and next_expected <= datetime.utcnow() + timedelta(days=3):
            restock_soon.append({
                "item": item_query,
                "last_purchased": pattern.get("last_purchased"),
